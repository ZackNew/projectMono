...
<template>
  <div class="flex justify-center items-center bg-[#333333]">
    <div class="my-16">
      ...
      <form @submit.prevent="handleSubmit" class="flex flex-col items-center justify-center gap-4 mb-4">
        <input v-model="fullName" type="text" placeholder="Full Name:"
          class="border-[1px] border-[#333333] w-[360px] rounded-full py-2 px-6 button-shadow-black bg-[#E9E3D3] text-xs focus:outline-none focus:ring-1 focus:ring-gray-500" />
        
        <div class="flex items-center gap-2">
          <input v-model="phone" type="text" placeholder="Phone Num:"
            class="border-[1px] border-[#333333] w-[235px] rounded-full py-2 px-6 button-shadow-black bg-[#E9E3D3] text-xs focus:outline-none focus:ring-1 focus:ring-gray-500" />
          <input v-model="email" type="text" placeholder="Email:"
            class="border-[1px] border-[#333333] w-[235px] rounded-full py-2 px-6 button-shadow-black bg-[#E9E3D3] text-xs focus:outline-none focus:ring-1 focus:ring-gray-500" />
        </div>

        <AppDropDown width="w-[506px]" :options="[
          { label: 'Option A', value: 'a' },
          { label: 'Option B', value: 'b' }
        ]" placeholder="How skill are you in design?" @select="val => designSkill = val" />

        <AppDropDown width="w-[476px]" :options="[
          { label: 'Option A', value: 'a' },
          { label: 'Option B', value: 'b' }
        ]" placeholder="How do you want to use your learned skill afterwards?" @select="val => futureUse = val" />

        <AppDropDown width="w-[367px]" :options="[
          { label: 'Option A', value: 'a' },
          { label: 'Option B', value: 'b' }
        ]" placeholder="where did you hear about us?" @select="val => hearFrom = val" />

        <AppDropDown width="w-[248px]" :options="[
          { label: 'Option A', value: 'a' },
          { label: 'Option B', value: 'b' }
